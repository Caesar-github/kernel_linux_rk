...
struct es7210_priv *resume_es7210;//CSQERR
...
static int es7210_multi_chips_write(u8 reg, unsigned char value)
{
	u8 i;
	for (i = 0; i < ADC_DEV_MAXNUM; i++) {
		if (i2c_clt1[i]->addr == 0x42 && (reg == 0x1B ||
                   reg == 0x1C ||
                   reg == 0x1D ||
                   reg == 0x1E)) {
            //es7210_write(0x16,0x00,i2c_clt1[i]);// for ALC DISABLE
            es7210_write(reg,0xBF,i2c_clt1[i]);
            return 0;
        } else if (i2c_clt1[i]->addr == 0x40 && (reg == 0x1B ||
                   reg == 0x1C ||
                   reg == 0x1D ||
                   reg == 0x1E)) {
			es7210_write(reg,0xBF,i2c_clt1[i]);
								...
								struct snd_soc_dai *dai)
{
	...
	printk("enter into %s,es7210_init_reg %d\n", __func__, es7210_init_reg);
	if (es7210_init_reg == 0) {
			printk("enter into %s  schedule_delayed_work\n", __func__);
			schedule_delayed_work(&es7210->pcm_pop_work, msecs_to_jiffies(100));
	}
	return 0;
}

int es7210_pcm_startup_extern(void)
{
	printk("enter into %s,es7210_init_reg %d\n", __func__, es7210_init_reg);
	if (es7210_init_reg == 0) {
			printk("enter into %s  schedule_delayed_work\n", __func__);
...
int es7210_pcm_hw_params_extern(void)
{
	int i;
	printk("enter into %s\n", __func__);
...
EXPORT_SYMBOL(es7210_pcm_startup_extern);
EXPORT_SYMBOL(es7210_pcm_hw_params_extern);
...
{
	int i;
	printk("enter into %s,params_format(params) is %d\n", __func__, params_format(params));
	es7210_multi_chips_update_bits(ES7210_RESET_CTL_REG00, 0x30, 0x30);
	for (i = 0; i < ADC_DEV_MAXNUM; i++) {
			/* set es7210 bit size */
...
static int es7210_probe(struct snd_soc_component *component)
{
...
#else
	component->control_data = devm_regmap_init_i2c(es7210->i2c, &es7210_regmap_config);
	...
	printk("begin->>>>>>>>>>%s!\n", __func__);
	//snd_soc_component_init_regmap(component, es7210->regmap);
	tron_component1[es7210_codec_num++] = component;
	INIT_DELAYED_WORK(&es7210->pcm_pop_work, pcm_pop_work_events);
	es7210_tdm_init_codec(es7210->tdm_mode);

	//snd_soc_add_component_controls(component, es7210_snd_controls,
	//							   ARRAY_SIZE(es7210_snd_controls));
	es7210_pcm_startup_extern();
	es7210_pcm_hw_params_extern();
	return 0;
}
							...
							const struct i2c_device_id *i2c_id)
{
	...
	if (i2c_id->driver_data < ADC_DEV_MAXNUM) {
		...
		ret = devm_snd_soc_register_component(&i2c->dev, &soc_codec_dev_es7210,
									 es7210_dai[i2c_id->driver_data], 1);
		if (ret < 0) {
			printk("%s devm_snd_soc_register_component error!\n", __func__);
			kfree(es7210);
			return ret;
		}
	}
