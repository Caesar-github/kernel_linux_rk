...
static int techpoint_get_regulators(struct techpoint *techpoint)
{
	...
	if (!techpoint->supplies)
		techpoint->supplies = devm_kzalloc(dev,
			sizeof(struct regulator_bulk_data) * TECHPOINT_NUM_SUPPLIES,
			GFP_KERNEL);
...
static int techpoint_analyze_dts(struct techpoint *techpoint)
{
	...
	ret = regulator_bulk_enable(TECHPOINT_NUM_SUPPLIES, techpoint->supplies);
	if (ret < 0)
...
static int techpoint_initialize_controls(struct techpoint *techpoint)
{
	...
	if (techpoint->input_type == TECHPOINT_DVP_BT1120) {
		...
		if (ret)
			return ret;
		handler->lock = &techpoint->mutex;
		techpoint->link_freq_ctrl = v4l2_ctrl_new_int_menu(handler, NULL,
					V4L2_CID_LINK_FREQ,
					0, 0,
		...
		pixel_rate = (u32)mode->link_freq_value / mode->bpp * 2 * mode->lane;
		techpoint->pixel_rate_ctrl = v4l2_ctrl_new_std(handler, NULL,
			V4L2_CID_PIXEL_RATE, 0, pixel_rate, 1, pixel_rate);
		dev_dbg(dev, "initialize pixel_rate %lld\n", pixel_rate);
	}
...
static int __techpoint_power_on(struct techpoint *techpoint)
{
	...
	if (!IS_ERR(techpoint->xvclk)) {
		ret = clk_set_rate(techpoint->xvclk, techpoint->xvclk_freq_value);
		if (ret < 0)
			dev_warn(dev, "Failed to set xvclk rate\n");
		if (clk_get_rate(techpoint->xvclk) != techpoint->xvclk_freq_value)
		      ...
		      struct v4l2_subdev_format *fmt)
{
	...
	for (i = 0; i < techpoint->video_modes_num; i++) {
		dist = techpoint_get_reso_dist(&techpoint->video_modes[i], framefmt);
				 ...
				 struct v4l2_mbus_config *cfg)
{
	...
	if (techpoint->input_type == TECHPOINT_DVP_BT1120) {
		...
		cfg->flags = V4L2_MBUS_BT1120_CHANNELS | \
				   ...
				   struct rkmodule_inf *inf)
{
	...
	strlcpy(inf->base.module, techpoint->module_name, sizeof(inf->base.module));
...
static long techpoint_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_GET_MODULE_INFO:
		...
		break;
	case RKMODULE_GET_DVP_CHANNEL_NUMS:
		*(__u32 *)arg = techpoint->channel_nums;
		break;
	case RKMODULE_GET_VC_FMT_INFO:
		if (!techpoint->streaming)
			techpoint_get_vc_fmt_inf(techpoint, (struct rkmodule_vc_fmt_info *)arg);
		else
			__techpoint_get_vc_fmt_inf(techpoint, (struct rkmodule_vc_fmt_info *)arg);
		break;
	case RKMODULE_GET_VC_HOTPLUG_INFO:
			techpoint_get_vc_hotplug_inf(techpoint, (struct rkmodule_vc_hotplug_info *)arg);
		break;
	case RKMODULE_GET_START_STREAM_SEQ:
		*(int *)arg = RKMODULE_START_STREAM_FRONT;
		break;
	case RKMODULE_GET_VICAP_RST_INFO:
		techpoint_get_vicap_rst_inf(techpoint, (struct rkmodule_vicap_reset_info *)arg);
		break;
	case RKMODULE_SET_VICAP_RST_INFO:
		techpoint_set_vicap_rst_inf(techpoint, *(struct rkmodule_vicap_reset_info *)arg);
		break;
	default:
		ret = -ENOTTY;
		break;
				   ...
				   unsigned int cmd, unsigned long arg)
{
	...
	case RKMODULE_GET_VC_HOTPLUG_INFO:
		...
		break;
	case RKMODULE_GET_DVP_CHANNEL_NUMS:
		ret = techpoint_ioctl(sd, cmd, &ch_nums);
		if (!ret)
			ret = copy_to_user(up, &ch_nums, sizeof(ch_nums));
		break;
	case RKMODULE_GET_VICAP_RST_INFO:
	...
	case RKMODULE_GET_START_STREAM_SEQ:
		...
		ret = techpoint_ioctl(sd, cmd, seq);
		if (!ret)
			 ...
			 const struct i2c_device_id *id)
{
	...
	techpoint->pad.flags = MEDIA_PAD_FL_SOURCE;
	sd->entity.function = MEDIA_ENT_F_CAM_SENSOR;
	ret = media_entity_pads_init(&sd->entity, 1, &techpoint->pad);
	if (ret < 0)
		goto err_power_off;
#endif
