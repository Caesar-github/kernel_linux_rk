...
static int check_chip_id(struct techpoint *techpoint)
{
	...
	if (chip_id_h == TP9930_CHIP_ID_H_VALUE && chip_id_l == TP9930_CHIP_ID_L_VALUE) {
	dev_info(&client->dev, "techpoint check chip id CHIP_TP9930 !\n");
		techpoint->chip_id = CHIP_TP9930;
		techpoint->input_type = TECHPOINT_DVP_BT1120;
	return 0;
	} else if (chip_id_h == TP2855_CHIP_ID_H_VALUE && chip_id_l == TP2855_CHIP_ID_L_VALUE) {
	dev_info(&client->dev, "techpoint check chip id CHIP_TP2855 !\n");
		techpoint->chip_id = CHIP_TP2855;
		techpoint->input_type = TECHPOINT_MIPI;
	return 0;
	}  else if (chip_id_h == TP2815_CHIP_ID_H_VALUE && chip_id_l == TP2815_CHIP_ID_L_VALUE) {
	dev_info(&client->dev, "techpoint check chip id CHIP_TP2815 !\n");
		techpoint->chip_id = CHIP_TP2855;
		techpoint->input_type = TECHPOINT_MIPI;
	return 0;
	} else {
...
static int detect_thread_function(void *data)
{
	...
	down(&reg_sem);
		if (techpoint->chip_id == CHIP_TP9930) {
			tp9930_get_all_input_status(client, techpoint->detect_status);
			for (i = 0; i < PAD_MAX; i++)
				tp9930_set_decoder_mode(client, i, techpoint->detect_status[i]);
		} else if (techpoint->chip_id == CHIP_TP2855)
			tp2855_get_all_input_status(client, techpoint->detect_status);
	up(&reg_sem);
	...
	down(&reg_sem);
		if (techpoint->power_on) {
			for (i = 0; i < PAD_MAX; i++) {
			if (techpoint->chip_id == CHIP_TP9930)
					detect_status = tp9930_get_channel_input_status(client, i);
			else if (techpoint->chip_id == CHIP_TP2855)
					detect_status = tp2855_get_channel_input_status(client, i);

				if ((techpoint->detect_status[i]) != (detect_status)) {
					if (!detect_status)
						dev_err(&client->dev, "detect channel %d video plug out\n", i);
					else
						dev_err(&client->dev, "detect channel %d video plug in\n", i);
...
static int __maybe_unused detect_thread_start(struct techpoint *techpoint)
{
	...
	techpoint->detect_thread = kthread_create(detect_thread_function,
				   techpoint, "techpoint_kthread");
	if (IS_ERR(techpoint->detect_thread)) {
		dev_err(&client->dev, "kthread_create techpoint_kthread failed\n");
				       ...
				       struct rkmodule_vc_hotplug_info *inf)
{
	...
	for (ch = 0; ch < 4; ch++) {
		if (techpoint->chip_id == CHIP_TP9930)
			detect_status = tp9930_get_channel_input_status(client, ch);
		else if (techpoint->chip_id == CHIP_TP2855)
			detect_status = tp2855_get_channel_input_status(client, ch);
